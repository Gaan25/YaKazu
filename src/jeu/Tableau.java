...
/////////////////////ALGORITHME DU BACKTRACKING///////////////////////////
	...
	public boolean estValide (int position,long timeOut) {
		...
		//On y a ajout� un timeOut pour optimiser le temps d'execution celon les cas ou la grille est trop longue � raisoudre
		if (System.currentTimeMillis() - timeOut > 10000){
			return false;
		}
		// Si on est a la n*n eme case (on sort du tableau), on s'arrete ici car le parcours aura �t� fait en entier
		...
		{
			if ((absentLigneCourante(k,i,position)) && (absentColonneCourante(k,j,position))  && (taille_valide(k,i,j,position)) ) //On a besoin de la position pr�cise a cause des cases noires//
			{
				modifier_case(i,j,k);
	...
	public boolean taille_valide (int k , int i ,int j ,int position){
		if (k > taille_ligne_courante(i,position) || (k > taille_colonne_courante(j,position))) // Car un chiffre ne peut pas �tre plus grand que la taille max de la colonne/ligne dans laquelle il se situe.
			return false;
		else return true;
